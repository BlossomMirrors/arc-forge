...
Approving a Flatpak submission runs a generated script over SSH on the configured remote host (default `repo.blossomos.org`, configurable in Infra Settings). Before that can work, a few one-time things need to be set up **on the remote host itself**. Forge doesn't and can't automate these:
...
3. Install `rclone` and FUSE (`fusermount`/`mountpoint`) on the remote host. The OSTree repo itself lives in a Cloudflare R2 bucket, not on the remote host's local disk (bucket/path configurable in Infra Settings). Every publish/repair/unpublish script mounts that bucket with `rclone mount` for the duration of the run, using Forge's own `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`, and unmounts it again before exiting, nothing is written to an `rclone.conf` on the remote host. If the configured bucket/path has no repo yet, it needs to be `ostree init --mode=archive-z2`'d once, same "Forge assumes it's already there" posture as the runtimes/tooling below.

The GPG signing key itself is uploaded in Infra Settings (paste the ASCII-armored private key), not assumed to already exist on the remote host. Every publish/repair/unpublish run imports it fresh and derives its fingerprint straight from the key file, so the remote host's own keyring is disposable, losing it doesn't mean losing the signing key.
