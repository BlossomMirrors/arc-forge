...
3. **For repair/unpublish only** (not publish, see below): install `rclone` and FUSE (`fusermount`/`mountpoint`) on the host itself. Those two scripts still mount the R2 bucket directly with `rclone mount` for the duration of the run, using Forge's own `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`, and unmount it again before exiting - nothing is written to an `rclone.conf` on the host for this.
4. **For publish specifically**: R2 is *not* mounted by Forge directly at all anymore (added 2026-08-07, after establishing that a host-side mount bind-mounted into the container doesn't actually isolate anything from whatever the host filesystem is doing - same lesson as the `$STAGING_REPO` fix above). Instead, [rclone's Docker/Podman volume plugin](https://rclone.org/docker/) must be running as its own persistent service on this host (`rclone serve docker --socket-addr ...`, *not* `docker plugin install`, which is Docker-managed-plugin-only and doesn't apply to Podman), with its Unix socket registered in **root's own** `/etc/containers/containers.conf` under `[engine.volume_plugins]` (not `buildUser`'s/`remoteUser`'s per-user config - `sudo docker volume create` needs to see it there). `buildRemoteScript` then creates a fresh, uniquely-named volume per run via `sudo docker volume create --driver rclone ...` and destroys it again in its trap - the plugin owns the actual FUSE mount inside its own daemon, the container just gets a plain named volume. **This exact setup (rclone's unmanaged `serve docker` mode registered with Podman's `containers.conf`) has not been verified against a real install** - the protocol-level compatibility is plausible (Podman's plugin config expects a path to a Unix socket, which is what `rclone serve docker` provides) but untested; expect to debug the systemd unit / socket registration / actual `docker volume create` call the first time this runs for real.

The OSTree repo itself lives in a Cloudflare R2 bucket, not on any host's local disk (bucket/path configurable in Infra Settings). If the configured bucket/path has no repo yet, it needs to be `ostree init --mode=archive-z2`'d once, same "Forge assumes it's already there" posture as the runtimes/tooling below.
